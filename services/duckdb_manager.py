...
class DuckDBManager:
    ...
    def _init_schema(self):
        """Initialize base schema for Geotab data."""
        self.conn.execute("""
            CREATE TABLE IF NOT EXISTS devices (
                id VARCHAR,
                name VARCHAR,
                serialNumber VARCHAR,
                vehicleIdentificationNumber VARCHAR,
                deviceType VARCHAR,
        ...
        # Updated 'events' schema to include coordinates if available (LogRecord join or from event data if enriched)
        # Note: ExceptionEvent object itself doesn't have lat/lon, but we can join with LogRecord based on time.
        # For this demo, we'll try to store lat/lon if we can fetch it during preload.
        self.conn.execute("""
